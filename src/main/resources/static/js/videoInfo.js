/**
 * Created by luweiying on 2018/5/28 0028.
 */
$(function () {
 ...
 * 文章详情页数据
 * */
function initVideoInfo(videoId) {
    id = videoId;
    var param = {id: videoId};
    $.ajax({
        url: '/recordVideo/getVideoInfo',
        type: 'POST',//请求方式
        async: true,
        ...
        success: function (data) {//成功执行200
            if (data.respCode == 200) {
                //doT.js模版引擎渲染
                var infoData = doT.template($("#videoInfoTemplate").html());
                $('#videoInfoData').html(infoData(data.data));
